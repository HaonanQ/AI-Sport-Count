...
def sport(videos,model): # num为是使用的视频,model为使用的模式，1为并行（左右），2为并列（上下）
    ...
    video = cv2.VideoCapture(videopaths) # video_path+number+'_.mp4'
    ...
    while cap.isOpened():
        try:
            ...
            print("发生某些错误，摄像头可能未打开或视频文件损坏！")
            ttk.dialogs.dialogs.Messagebox.ok(title='提示', message="发生某些错误，摄像头可能未正确打开或视频文件损坏！\n"
                                              "请重新尝试。")
            cap.release()
            video.release()
